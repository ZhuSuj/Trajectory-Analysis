var parseDate = d3v0.time.format("%d/%m/%Y %H:%M:%S").parse;
var dataset;
var dataset_nested = [];
var hierarchy;
var root_key = "R0";
...
var data_type = "";
var nest_by_name = d3v0.nest().key(function(d) {return d.value}); //d[1] Group by key
var colores_d3v0 =  d3v0.scale.category10();

var customTimeFormat = d3v0.time.format.multi([ 
...
//	console.log(d.name)
	...
	if(d.depth == 1){
		num_initial_color < 5 ? color_root = colores_d3v0(index) : color_root = colores_brewer(index);
		//color_root = colores_d3v0(index);
...
//		console.log(d.text); 
		return {"date":parseDate(d.date_time), "value": d.value, "text":d.text, "volume":d.volume };
