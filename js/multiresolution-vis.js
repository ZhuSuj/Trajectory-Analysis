...
var nest_by_key = d3v0.nest().key(function(d) {return d.key}); //Group by key
var stack = d3v0.layout.stack()
...
var eventWidthSizeLabelScale=0.9;
var eventHeightSizeLabelScale=0.9;
...
var clickriverrectid = ""
d3.select("#containerfloor1").on("click",function(d) {
	d3.select("#containerfloor1").select(".RectSensoridfloor"+ clickriverrectid)
    .attr("fill","white").style("opacity", 0.1)
})
...
function updateFlows(){
    ...
    //--MULTIRESOLUTION---
    dataCurrentlyMultiresolutionTop = stack(nest_by_key.entries(nivel_bajo));//Used in Focus Local
	...
	//Update Axis
	var max = d3v0.max(nivel_alto, function(d) {return d.y0 + d.y;})
	...
	//Update Axis
	var max = d3v0.max(dataCurrentlyFocus, function(aray){
		return d3v0.max(aray.values,function(d){return d.y0+d.y;});
	})
...
function loadMultiresolutionVis(){
	...
	d3v0.select("#fading-colors").on("change", function() {
		opts.fadingColors = document.getElementById("fading-colors").checked;
		callUpdate();
	});
	
	d3v0.select("#limit-ranges").on("change", function() {
	...
	d3v0.select("#alpha").on("input", function() {
	...
	d3v0.select("#alpha").on("mouseover", function() {
		brushZoomOver();
	});
	d3v0.select("#alpha").on("mouseout", function() {
		brushOutAll();
	});
	...
	d3v0.select("#beta").on("input", function() {
	...
	d3v0.select("#beta").on("mouseover", function() {
		brushDisLeftOver();
		brushDisRightOver();
	});
	d3v0.select("#beta").on("mouseout", function() {
		brushOutAll();
	});
	//
	//
	...
	d3v0.select("#gamma").on("input", function() {
	...
	d3v0.select("#gamma").on("mouseover", function() {
		brushNorLeftOver();
		brushNorRightOver();
	});
	d3v0.select("#gamma").on("mouseout", function() {
		brushOutAll();
	});
	//
	//
	d3v0.select("#col-visualization").selectAll("input").on("change", function(){
	...
	d3v0.select("#candadoLeft").on("click",function(d){
		...
		if(document.getElementById("candadoLeft").classList.contains("consin")){
			d3v0.select("#candadoLeft").attr('xlink:href',opts.pathCandadoOpen);
			lockedLeft = false;
		}else{
			d3v0.select("#candadoLeft").attr('xlink:href',opts.pathCandadoClose);
			lockedLeft = true;
		}
	})

	d3v0.select("#candadoRight").on("click",function(d){
		...
		if(document.getElementById("candadoRight").classList.contains("consin")){
			d3v0.select("#candadoRight").attr('xlink:href',opts.pathCandadoOpen);
			lockedRight = false;
		}else{
			d3v0.select("#candadoRight").attr('xlink:href',opts.pathCandadoClose);
			lockedRight = true;
		}
	})
...
//COLORS AVAILABLES
//d3v0.scale.category10() d3v0 category10 BETTER
//d3v0.scale.category20() d3v0 category20
//d3v0.scale.category20b() d3v0 category20b BETTER
//d3v0.scale.category20c() d3v0 category20c
...
//	d3v0.selectAll('div.svg-container svg')
...
//	  .attr( 'fill', 'purple' );
	...
	/* SVG */
	svg_multiresolution_vis = d3v0.select("body").select("#multiresolution-vis")
...
//						.style("overflow","visible")
	...
	yScaleFocus = d3v0.scale.linear().range([ heightFocus, 0 ]);

	yAxisFocus = d3v0.svg.axis().scale(yScaleFocus)
							.tickSize(5, 0)
							.tickPadding(5)
							.ticks(10)
							.orient("left");
	...
	context.append("text")
			...
			.text(function(){
				if(opts.x_axis_label_context == "Conference Data"){
					d3.select("this").attr("x","10")
					return "Data"
				}
				else {return opts.x_axis_label_context}
			});


	xScaleContext = d3v0.time.scale().range([ 0, (width) ]);
	yScaleContext = d3v0.scale.linear().range([ heightContext, 0 ]);

	xAxisContext = d3v0.svg.axis().scale(xScaleContext)
								 .tickSize(5, 0) //inner and outer : extrems of axis
								 .tickPadding(5)
//								 .tickSize(-heightContext, 0) //inner and outer : extrems of axis
//								 .tickPadding(d3v0.time.minute, 1)
								 .orient("bottom");
	//
	yAxisContext = d3v0.svg.axis().scale(yScaleContext)
								.tickSize(-width, 0)
								.tickPadding(5)
								.orient("left");
...
function display() {
	dateExtRange = d3v0.extent(dataset, function(d) {return d.date;}); // max and min date
...
//	console.log("nTimeGranularity" + nTimeGranularity)
	...
	switch (timePolarity) {
		case 0: 
			dateMinRange = d3v0.time.minute.floor(dateExtRange[0]), // min date
			dateMaxRange = d3v0.time.minute.ceil(dateExtRange[1]); // max date
			dateRange = d3v0.time.minutes(dateMinRange,dateMaxRange, nTimeGranularity);
			break;
		case 1: 
			dateMinRange = d3v0.time.hour.floor(dateExtRange[0]), // min date
			dateMaxRange = d3v0.time.hour.ceil(dateExtRange[1]); // max date
			dateRange = d3v0.time.hours(dateMinRange,dateMaxRange, nTimeGranularity);
			break;
		case 2: 
			dateMinRange = d3v0.time.day.floor(dateExtRange[0]), // min date
			dateMaxRange = d3v0.time.day.ceil(dateExtRange[1]); // max date
			dateRange = d3v0.time.days(dateMinRange,dateMaxRange, nTimeGranularity);
			break;
		case 3: 
			dateMinRange = d3v0.time.week.floor(dateExtRange[0]), // min date
			dateMaxRange = d3v0.time.week.ceil(dateExtRange[1]); // max date
			dateRange = d3v0.time.weeks(dateMinRange,dateMaxRange, nTimeGranularity);
			break;
		case 4: 
			dateMinRange = d3v0.time.month.floor(dateExtRange[0]), // min date
			dateMaxRange = d3v0.time.month.ceil(dateExtRange[1]); // max date
			dateRange = d3v0.time.months(dateMinRange,dateMaxRange, nTimeGranularity);
			break;
		case 5: 
			dateMinRange = d3v0.time.year.floor(dateExtRange[0]), // min date
			dateMaxRange = d3v0.time.year.ceil(dateExtRange[1]); // max date
			dateRange = d3v0.time.years(dateMinRange,dateMaxRange, nTimeGranularity);
			break;
	}
...
//	hierarchy.reverse();
	...
	yScaleFocus.domain([ 0, d3v0.max(nivel_bajo, function(d) {return d.y0 + d.y;}) ]);
...
//	console.log("comienza a crear")
	...
	switch (timePolarity) {
		case 0: 
			...
			xScaleContext.nice(d3v0.time.minute);
			break;
		case 1: 
			xAxisContext.ticks(10).tickFormat(customTimeFormat);
			xScaleContext.nice(d3v0.time.hour);
			break;
		case 2: 
			xAxisContext.ticks(10).tickFormat(customTimeFormat);
			xScaleContext.nice(d3v0.time.day);
			break;
		case 3: 
			xAxisContext.ticks(10).tickFormat(customTimeFormat);
			xScaleContext.nice(d3v0.time.week);
			break;
		case 4: 
			xAxisContext.ticks(10).tickFormat(customTimeFormat);
			xScaleContext.nice(d3v0.time.month);
			break;
		case 5: 
			xAxisContext.ticks(10).tickFormat(customTimeFormat);
			xScaleContext.nice(d3v0.time.year);
			break;
	}
    ...
    d3v0.select("body").select("#global")
        .attr("position","absolute").style("top", "515px")
        // .style("left","457px")
    // .style("width","1450px")
    ...
    d3v0.select("body").select("#multiresolution-vis")
        .attr("position","absolute").attr("height","356px")
    d3.selectAll(".cm-container-white").style("box-shadow","none").style("border-bottom","none")
    // d3.selectAll("#x_axis_focus").remove()
...
//	console.log(brushContextNorRight.extent());
	...
	//	  .each(function() { combinedD += d3v0.select(this).attr("d"); });
	//	context.append("path")
	//		.attr("class","area")
	//	  .attr("d", combinedD)
	//	  .style({"fill" : "red"},
	...
	for(var index=1;index<4;index++){
		focus.select("#flowsInFocus").selectAll(".focus.area" + index) //Focus Area BOTTOM hierarchy
					...
					.attr("class", function(d){
						return "focus area" +index +" "+  d.key
						+ " RectSensoridfloor" + d.name})
					.attr("d", function(d) {return areaFocus(d, index);})
					.style({
							"fill" : function(d) {
								switch (index) {
...
//	console.log("finish createTextLabel")
	/* ToolTip */
	tooltip = d3v0.select("body").append("div")
...
function getTextLabels(){
	...
	switch (timePolarity) {
		case 0:
			if(dateLimMax > d3v0.time.minute.offset(dateMaxRange, -2)){
				dateLimMax = d3v0.time.minute.offset(dateMaxRange, -2);
			}
			break;
		case 1:
			if(dateLimMax > d3v0.time.hour.offset(dateMaxRange, -2)){
				dateLimMax = d3v0.time.hour.offset(dateMaxRange, -2);
			}
			break;
		case 2:
			if(dateLimMax > d3v0.time.day.offset(dateMaxRange, -2)){
				dateLimMax = d3v0.time.day.offset(dateMaxRange, -2);
			}
			break;
		case 3:
			if(dateLimMax > d3v0.time.week.offset(dateMaxRange, -2)){
				dateLimMax = d3v0.time.week.offset(dateMaxRange, -2);
			}
			break;
		case 4:
			if(dateLimMax > d3v0.time.month.offset(dateMaxRange, -2)){
				dateLimMax = d3v0.time.month.offset(dateMaxRange, -2);
			}
			break;
		case 5:
			if(dateLimMax > d3v0.time.year.offset(dateMaxRange, -2)){
				dateLimMax = d3v0.time.year.offset(dateMaxRange, -2);
			}
			break;
	}
	...
	scaleTextLabel = d3v0.scale.linear()
							.clamp(false)
...
function getPointAnomalyByPercentage(timeSerieValues,percentageIncrease,percentageDecrease){
    ...
    for (let i=1;i<timeSerieValues.length;i++){
        ...
        var max=0;
        var min=100000000;
        if(currVariation>max)max=currVariation;
        if(currVariation<min)min=currVariation;
...
function getPointChanges(dataInZoomArea,multiresolutionType,yScale){
        ...
        layerInZoom.values.forEach(function(element){
            ...
            if(currMatchPointDetection.length==1){
                let nuevoElemento = {
					...
					"Variation":currMatchPointDetection[0].Variation,
					"MaxVariation":currMatchPointDetection[0].MaxVariation,
					"MinVatiation":currMatchPointDetection[0].MinVariation,
    ...
    let linear1=d3.scaleLinear()
        .domain([0,150])
        .range([getPxFromRem(eventWidthSizeLabelScale),getPxFromRem(eventWidthSizeLabelScale)+20])
    let linear2=d3.scaleLinear()
        .domain([0,150])//frequent2的数据2500，volume的数据150
        .range([getPxFromRem(eventHeightSizeLabelScale),getPxFromRem(eventHeightSizeLabelScale)+20])
    ...
    //update
    points.attr("x", function(d) {return scalesFocus[selectAxisFocus(d.date)](d.date)-getPxFromRem(eventWidthSizeLabelScale/2); })
        .attr("y", function(d) {return yScale(d.y0 + d.value/2)-getPxFromRem(eventHeightSizeLabelScale/2); });

    //enter
    // points.enter().append("rect")
    //     .attr("class",function(d){
    //         var timeformattemp = d3.timeFormat("%Y-%m-%d %H:%M:%S");
    //         var timeformat = timeformattemp  (d.date)
    //             .replace(" ","-").replace(":","-").replace(":","-")
    //
    //         var region = "RectSensoridfloor"
    //         for(var t = 0 ; t<nivel_bajo.length; t++){
    //             if(nivel_bajo[t].key == d.key){
    //                 region = region + nivel_bajo[t].name
    //                 break;
    //             }
    //         }
    //         return "point-detection "+ "updownflag"+region +" " +timeformat
    //     })
    //     .attr("id", function(d){
    //         var timeformattemp = d3.timeFormat("%Y-%m-%d %H:%M:%S");
    //         var timeformat = timeformattemp  (d.date)
    ...
    //             }
    //         }
    //         return region + "_" + timeformat
    //     })
    //     .attr("x", function(d) {return scalesFocus[selectAxisFocus(d.date)](d.date)-getPxFromRem(eventWidthSizeLabelScale/2); })
    //     .attr("y", function(d) {return yScale(d.y0 + d.value/2)-getPxFromRem(eventHeightSizeLabelScale/2); })
    //     .attr("width",getPxFromRem(eventWidthSizeLabelScale)-5)
    //     .attr("height",getPxFromRem(eventHeightSizeLabelScale))
		// .attr("fill",function (d) {
		// 	if(d.relationRelative==1)return linear1(d.Variation);
		// 	else if(d.relationRelative==0)return linear2(d.Variation);
    //     })
    //     .style({
    //         "pointer-events":"none",
    //         "opacity":0
    ...
    //     })
    // var svgupimage
    // $.get('img/events/arrow_down.svg', function(data){
    //     svgupimage = data.documentElement
    //     $('#containerfloor1').append(data.documentElement)
    // })
    //
    // points.enter().forEach(function(d){console.log(d)})
    ...
    points.enter()
        ...
        .style("x", function(d) {return scalesFocus[selectAxisFocus(d.date)](d.date)-linear2(d.Variation)/2; })
        .style("y", function(d) {
        	return yScale(d.y0 + d.value/2)-linear2(d.Variation); })
        .attr("xlink:href",d=>{return d.pathImg;})
        .attr("width",function (d) {
			return linear2(d.Variation)
        })
        .attr("height",function (d) {
			return linear2(d.Variation)
        })
		.attr("Variation",function (d) {return d.Variation})
        .style({
            "pointer-events":"none",
            "opacity":0
        })
	...
	// points.append("text")
	// 	.text(function (d) {
	// 		return d.Variation.toFixed(2)
     //    })
     //    .attr("x", function(d) {return scalesFocus[selectAxisFocus(d.date)](d.date)-getPxFromRem(eventWidthSizeLabelScale/2)-10; })
     //    .attr("y", function(d) {return yScale(d.y0 + d.value/2)-getPxFromRem(eventHeightSizeLabelScale/2)-10; })
     //    .style({
     //        "pointer-events":"none",
     //        "opacity":0
     //    })
...
var tooltipEvents = d3.select("body").append("div")
...
function pointChangeMouseOver(d){
    ...
    showToolTipEvents(d.Variation,tooltipTopPosition);
}
...
function getTooltipLeftPosition(){
    let left_position;
    let with_tooltip = $("#tooltip-flow").width();
...
function execAlgosInZoomArea(){
    ...
    getTextLabels();
    // //point detections
    getPointChanges(dataInZoomArea,focus,yScaleFocus);
}
...
function createTooltip(){
    ...
    $( "#heatmapdisplaybytime" ).click(function(){
        d3.select("#heatmapdisplaybytime").attr("class","glyphicon glyphicon-pause")

        if( displayflag == 0 ){
            d3.select(".focus").append("g").append("rect").attr("id", "recttodisplayheatmap")
                .attr("width","10px").attr("height","30px")
                .attr("fill","red").attr("top", "500px").attr("left", "800px")
                .attr("position","absolute")
        }
        d3.select("#recttodisplayheatmap")
            .call(d3.drag()
                .on("start", function(d){
                ...
                .on("drag", function(d){
                    ...
                    var timeformattemp = d3.timeFormat("%d %H:%M:%S");
                    var heatmaptime = timeformattemp(mouseSelectedDate)

                    givetimereturnheamap("2019/7/"+ heatmaptime);
    ...
    focus.select("#flowsInFocus").selectAll(".focus")
		.attr("id", function(d){
			// console.log(d)
			// console.log(timeTooltip(scalesFocus[selectScaleFocusPixel(mousex)].invert(mousex)))
			return d.name
		})
		.attr("fill",function (d) {
			return d.color
			...
			.on("mouseover",function(d, i) {
                if(highlightflag){
                    d3.select("#containerfloor1").select("#room"+d.name)
                        .attr("fill",d.color).style("opacity", 1)
                    d3.select("#containerfloor1").select(".RectSensoridfloor"+d.name)
                        .attr("fill",d.color).style("opacity", 1)

                    //updowns
                    d3.selectAll(".point-detection").style("display","none")
                    d3.selectAll(".updownflagRectSensoridfloor"+d.name.toLowerCase())
                        .style("display","block")
                }
...
//					})
//					var bisectDate = d3v0.bisector(function(d) { return d; }).left; 
...
//						});
						...
						var formatDate;
//						timePolarity == 5 ? formatDate= d3v0.time.format("") : formatDate= d3v0.time.format("%d %b %Y");						
						switch(timePolarity){
							case 0: // 0 minutes
								formatDate= d3v0.time.format("%d %b %Y %H:%M");
								break;
							case 1:// 1 hours
								formatDate= d3v0.time.format("")
								break;
							case 2: // 2 days
								formatDate= d3v0.time.format("")
								break;
							case 3: //3 week
								...
								break;
							case 4: // 4 month
								formatDate= d3v0.time.format("%b %Y");
								break;
							case 5: //5 For years
								formatDate= d3v0.time.format("%Y")
								break;
						}
...
//									var second_line = "<p class='info'> At " + formatDate(dateSelected) + " " + customTimeFormat(dateSelected) + ", " + "<strong>" + valueSelected +" "+  data_type + "</strong></p>";
									var second_line = "<p class='info'> At " + formatDate(dateSelected) + ", " + "<strong>" +  data_type +": "+ valueSelected + "</strong></p>";
									var thrid_line = "";

									if(textsArraySelected.length!=0){
										thrid_line = "<p class='message'><strong>Click</strong> to see details"+"</p>";
									...
									if((d3v0.event.pageX + with_tooltip) >= screenWidth){
										left_position = d3v0.event.pageX - with_tooltip - 35;
									}else{
										left_position = d3v0.event.pageX;
									}

						tooltip.html(first_line + second_line + thrid_line)
									.style("left",(left_position + 10) + "px")
									.style("top",(d3v0.event.pageY + 15) + "px")
									;
			...
			.on("click",function(d){
                ...
                var heatmaptime = timeformattemp(mouseSelectedDate)
                givetimereturnheamap("2019/7/"+ heatmaptime);
                var nowunixtimestamp = "2014/06/"+ heatmaptime
                var datenow = (new Date(nowunixtimestamp)).getTime().toString()
                var datenew = datenow.substring(0,datenow.length-3)
                // vastheatmap("#VAST2015Garden",heatmapdata,datenew)
				...
				clickriverrectid = d.name


				d3.select("#containerfloor1").select(".mergedrect")
                        .attr("fill",meetinglayoutcolor).style("opacity", 0.1)
...
//点击河流之后，的弹出框
				// if(textsArraySelected.length!=0){
				// 	d3v0.select("#data-modal-title").html(titleDataModal)
				// 	d3v0.select("#data-modal-msg").html(msgDataModal)
				// 	d3v0.select(this).attr("data-toggle", "modal");
				// 	d3v0.select(this).attr("data-target", "#data-modal");
				// }
...
//			console.log(rangesDomainFocus[i].values[j].domain)
			...
			scalesFocus[scalesFocus.length] = d3v0.time.scale()
					.clamp(true) // 
			...
			axisFocus[axisFocus.length] = d3v0.svg.axis()
...
//			console.log(nTimeGranularity)
			switch (timePolarity) { 
				case 0:
					axisGridDivision = d3v0.svg.axis()
							.scale(scalesFocus[scalesFocus.length-1])
							.orient("top")
							.ticks(d3v0.time.minutes,nTimeGranularity)
				...
				case 1:
					axisGridDivision = d3v0.svg.axis()
							.scale(scalesFocus[scalesFocus.length-1])
							.orient("top")
							.ticks(d3v0.time.hours,nTimeGranularity)
							.tickSize(heightFocus)
							.tickFormat("");
					break;
				case 2:
					axisGridDivision = d3v0.svg.axis()
							.scale(scalesFocus[scalesFocus.length-1])
							.orient("top")
							.ticks(d3v0.time.days,nTimeGranularity)
							.tickSize(heightFocus)
							.tickFormat("");
					break;
				case 3:
					axisGridDivision = d3v0.svg.axis()
							.scale(scalesFocus[scalesFocus.length-1])
							.orient("top")
							.ticks(d3v0.time.weeks,nTimeGranularity)
							.tickSize(heightFocus)
							.tickFormat("");
					break;
				case 4:
					axisGridDivision = d3v0.svg.axis()
							.scale(scalesFocus[scalesFocus.length-1])
							.orient("top")
							.ticks(d3v0.time.months,nTimeGranularity)
							.tickSize(heightFocus)
							.tickFormat("");
					break;
				case 5:
					axisGridDivision = d3v0.svg.axis()
							.scale(scalesFocus[scalesFocus.length-1])
							.orient("top")
							.ticks(d3v0.time.years,nTimeGranularity)
							.tickSize(heightFocus)
							.tickFormat("");
					break;
			}
...
//			console.log("B")
			...
			switch (timePolarity) { 
				case 0://minutes
					if(i==1 || i==3){
						axisFocus[axisFocus.length - 1].ticks(d3v0.time.minutes, getNumIntervalsDistortion(timePolarity, i, j));
					}else{
				...
				case 1://hours
					if(i==1 || i==3){
						axisFocus[axisFocus.length - 1].ticks(d3v0.time.hours, getNumIntervalsDistortion(timePolarity, i, j));
					}else{
						axisFocus[axisFocus.length - 1].ticks(getNumberOfLabels(timePolarity, i, j));
					}
					break;
				case 2:
					if(i==1 || i==3){
						axisFocus[axisFocus.length - 1].ticks(d3v0.time.days, getNumIntervalsDistortion(timePolarity, i, j));
					}else{
						axisFocus[axisFocus.length - 1].ticks(getNumberOfLabels(timePolarity, i, j));
					}
					break;
				case 3:
					if(i==1 || i==3){
						axisFocus[axisFocus.length - 1].ticks(d3v0.time.weeks, getNumIntervalsDistortion(timePolarity, i, j));
					}else{
						axisFocus[axisFocus.length - 1].ticks(getNumberOfLabels(timePolarity, i, j));
					}
					break;
				case 4:
					if(i==1 || i==3){
						axisFocus[axisFocus.length - 1].ticks(d3v0.time.months, getNumIntervalsDistortion(timePolarity, i, j));
					}else{
						axisFocus[axisFocus.length - 1].ticks(getNumberOfLabels(timePolarity, i, j));
					}
					break;
				case 5:
					if(i==1 || i==3){
						axisFocus[axisFocus.length - 1].ticks(d3v0.time.years, getNumIntervalsDistortion(timePolarity, i, j));
					}else{
						axisFocus[axisFocus.length - 1].ticks(getNumberOfLabels(timePolarity, i, j));
					}
					break;
	...
	switch (timePolarity) {
		case 0:
			if(nTimeGranularity==1){
				return d3v0.time.minute.round(scalesFocus[selectScaleFocusPixel(mousex)].invert(mousex));
			}
			subHalf = d3v0.time.minute.offset(time, -nTimeGranularity);
			addHalf = d3v0.time.minute.offset(time, nTimeGranularity);
			moyenne = d3v0.time.minute.offset(d3v0.time.minutes(subHalf, addHalf, nTimeGranularity)[0], Math.floor(nTimeGranularity/2));
			if(time <= moyenne){
				return d3v0.time.minutes(subHalf, addHalf, nTimeGranularity)[0];
			}else{
				return d3v0.time.minutes(subHalf, addHalf, nTimeGranularity)[1];
			}
		case 1:
			if(nTimeGranularity==1){
				return d3v0.time.hour.round(scalesFocus[selectScaleFocusPixel(mousex)].invert(mousex));
			}
			subHalf = d3v0.time.hour.offset(time, -nTimeGranularity);
			addHalf = d3v0.time.hour.offset(time, nTimeGranularity);
			moyenne = d3v0.time.hour.offset(d3v0.time.hours(subHalf, addHalf, nTimeGranularity)[0], Math.floor(nTimeGranularity/2));
			if(time <= moyenne){
				return d3v0.time.hours(subHalf, addHalf, nTimeGranularity)[0];
			}else{
				return d3v0.time.hours(subHalf, addHalf, nTimeGranularity)[1];
			}
		case 2:
			if(nTimeGranularity==1){
				return d3v0.time.day.round(scalesFocus[selectScaleFocusPixel(mousex)].invert(mousex));
			}
			subHalf = d3v0.time.day.offset(time, -nTimeGranularity);
			addHalf = d3v0.time.day.offset(time, nTimeGranularity);
			moyenne = d3v0.time.day.offset(d3v0.time.days(subHalf, addHalf, nTimeGranularity)[0], Math.floor(nTimeGranularity/2));
			if(time <= moyenne){
				return d3v0.time.days(subHalf, addHalf, nTimeGranularity)[0];
			}else{
				return d3v0.time.days(subHalf, addHalf, nTimeGranularity)[1];
			}
		case 3:
			if(nTimeGranularity==1){
				return d3v0.time.week.round(scalesFocus[selectScaleFocusPixel(mousex)].invert(mousex));
			}
			subHalf = d3v0.time.week.offset(time, -nTimeGranularity);
			addHalf = d3v0.time.week.offset(time, nTimeGranularity);
			moyenne = d3v0.time.week.offset(d3v0.time.weeks(subHalf, addHalf, nTimeGranularity)[0], Math.floor(nTimeGranularity/2));
			if(time <= moyenne){
				return d3v0.time.weeks(subHalf, addHalf, nTimeGranularity)[0];
			}else{
				return d3v0.time.weeks(subHalf, addHalf, nTimeGranularity)[1];
			}
		case 4:
			if(nTimeGranularity==1){
				return d3v0.time.month.round(scalesFocus[selectScaleFocusPixel(mousex)].invert(mousex));
			}
			subHalf = d3v0.time.month.offset(time, -nTimeGranularity);
			addHalf = d3v0.time.month.offset(time, nTimeGranularity);
			moyenne = d3v0.time.month.offset(d3v0.time.months(subHalf, addHalf, nTimeGranularity)[0], Math.floor(nTimeGranularity/2));
			if(time <= moyenne){
				return d3v0.time.months(subHalf, addHalf, nTimeGranularity)[0];
			}else{
				return d3v0.time.months(subHalf, addHalf, nTimeGranularity)[1];
			}
		case 5:
			if(nTimeGranularity==1){
				return d3v0.time.year.round(scalesFocus[selectScaleFocusPixel(mousex)].invert(mousex));
			}
			subHalf = d3v0.time.year.offset(time, -nTimeGranularity);
			addHalf = d3v0.time.year.offset(time, nTimeGranularity);
			moyenne = d3v0.time.year.offset(d3v0.time.years(subHalf, addHalf, nTimeGranularity)[0], Math.floor(nTimeGranularity/2));
			if(time <= moyenne){
				return d3v0.time.years(subHalf, addHalf, nTimeGranularity)[0];
			}else{
				return d3v0.time.years(subHalf, addHalf, nTimeGranularity)[1];
			}
	}
}
...
function timeParser(time) {
	...
	switch (timePolarity) {
		case 0:
			subHalf = d3v0.time.minute.offset(d3v0.time.minute.round(time), -nTimeGranularity);
			addHalf = d3v0.time.minute.offset(d3v0.time.minute.round(time), nTimeGranularity);
			return d3v0.time.minutes(subHalf, addHalf, nTimeGranularity)[1];
		case 1:
			subHalf = d3v0.time.hour.offset(d3v0.time.hour.round(time), -nTimeGranularity);
			addHalf = d3v0.time.hour.offset(d3v0.time.hour.round(time), nTimeGranularity);
			return d3v0.time.hours(subHalf, addHalf, nTimeGranularity)[1];
		case 2:
			subHalf = d3v0.time.day.offset(d3v0.time.day.round(time), -nTimeGranularity);
			addHalf = d3v0.time.day.offset(d3v0.time.day.round(time), nTimeGranularity);
			return d3v0.time.days(subHalf, addHalf, nTimeGranularity)[1];
		case 3:
			subHalf = d3v0.time.week.offset(d3v0.time.week.round(time), -nTimeGranularity);
			addHalf = d3v0.time.week.offset(d3v0.time.week.round(time), nTimeGranularity);
			return d3v0.time.weeks(subHalf, addHalf, nTimeGranularity)[1];
		case 4:
			subHalf = d3v0.time.month.offset(d3v0.time.month.round(time), -nTimeGranularity);
			addHalf = d3v0.time.month.offset(d3v0.time.month.round(time), nTimeGranularity);
			return d3v0.time.months(subHalf, addHalf, nTimeGranularity)[1];
		case 5:
			subHalf = d3v0.time.year.offset(d3v0.time.year.round(time), -nTimeGranularity);
			addHalf = d3v0.time.year.offset(d3v0.time.year.round(time), nTimeGranularity);
			return d3v0.time.years(subHalf, addHalf, nTimeGranularity)[1];
	}
}
...
function brushContextMove() {
	...
	d3v0.select(this).call(brushContext.extent(extent1));		
	...
	switch (timePolarity) {
		case 0:
			facteurBrusDisLeft = (d3v0.time.minutes(brushContextDisLeft.extent()[0],brushContextDisLeft.extent()[1])).length;
			facteurBrusDisRight = (d3v0.time.minutes(brushContextDisRight.extent()[0],brushContextDisRight.extent()[1])).length;
			
			extentDisLeft = [d3v0.time.minute.offset(brushContext.extent()[0],-facteurBrusDisLeft), brushContext.extent()[0] ];
			extentDisRight = [brushContext.extent()[1],d3v0.time.minute.offset(brushContext.extent()[1],+facteurBrusDisRight) ];
			break;
		case 1:
			facteurBrusDisLeft = (d3v0.time.hours(brushContextDisLeft.extent()[0],brushContextDisLeft.extent()[1])).length;
			facteurBrusDisRight = (d3v0.time.hours(brushContextDisRight.extent()[0],brushContextDisRight.extent()[1])).length;
			
			extentDisLeft = [d3v0.time.hour.offset(brushContext.extent()[0],-facteurBrusDisLeft), brushContext.extent()[0] ];
			extentDisRight = [brushContext.extent()[1],d3v0.time.hour.offset(brushContext.extent()[1],+facteurBrusDisRight) ];

			break;
		case 2:
			facteurBrusDisLeft = (d3v0.time.days(brushContextDisLeft.extent()[0],brushContextDisLeft.extent()[1])).length;
			facteurBrusDisRight = (d3v0.time.days(brushContextDisRight.extent()[0],brushContextDisRight.extent()[1])).length;
			
			extentDisLeft = [d3v0.time.day.offset(brushContext.extent()[0],-facteurBrusDisLeft), brushContext.extent()[0] ];
			extentDisRight = [brushContext.extent()[1],d3v0.time.day.offset(brushContext.extent()[1],+facteurBrusDisRight) ];

			break;
		case 3:
			facteurBrusDisLeft = (d3v0.time.weeks(brushContextDisLeft.extent()[0],brushContextDisLeft.extent()[1])).length;
			facteurBrusDisRight = (d3v0.time.weeks(brushContextDisRight.extent()[0],brushContextDisRight.extent()[1])).length;
			
			extentDisLeft = [d3v0.time.week.offset(brushContext.extent()[0],-facteurBrusDisLeft), brushContext.extent()[0] ];
			extentDisRight = [brushContext.extent()[1],d3v0.time.week.offset(brushContext.extent()[1],+facteurBrusDisRight) ];
			break;
		case 4:
			facteurBrusDisLeft = (d3v0.time.months(brushContextDisLeft.extent()[0],brushContextDisLeft.extent()[1])).length;
			facteurBrusDisRight = (d3v0.time.months(brushContextDisRight.extent()[0],brushContextDisRight.extent()[1])).length;

			extentDisLeft = [d3v0.time.month.offset(brushContext.extent()[0],-facteurBrusDisLeft), brushContext.extent()[0] ];
			extentDisRight = [brushContext.extent()[1],d3v0.time.month.offset(brushContext.extent()[1],+facteurBrusDisRight) ];
			break;
		case 5:
			facteurBrusDisLeft = (d3v0.time.years(brushContextDisLeft.extent()[0],brushContextDisLeft.extent()[1])).length;
			facteurBrusDisRight = (d3v0.time.years(brushContextDisRight.extent()[0],brushContextDisRight.extent()[1])).length;
			
			extentDisLeft = [d3v0.time.year.offset(brushContext.extent()[0],-facteurBrusDisLeft), brushContext.extent()[0] ];
			extentDisRight = [brushContext.extent()[1],d3v0.time.year.offset(brushContext.extent()[1],+facteurBrusDisRight) ];
			break;
	}
	...
	switch (timePolarity) {
	case 0:
		facteurBrusNorLeft = (d3v0.time.minutes(brushContextNorLeft.extent()[0],brushContextNorLeft.extent()[1])).length;
		facteurBrusNorRight = (d3v0.time.minutes(brushContextNorRight.extent()[0],brushContextNorRight.extent()[1])).length;
		if(lockedLeft){
			extentNorLeft = [brushContextNorLeft.extent()[0], brushContextDisLeft.extent()[0] ];	
		}else{
			extentNorLeft = [d3v0.time.minute.offset(brushContextDisLeft.extent()[0],-facteurBrusNorLeft), brushContextDisLeft.extent()[0] ];
		}
		if(lockedRight){
			extentNorRight = [brushContextDisRight.extent()[1],brushContextNorRight.extent()[1] ];
		}else{
			extentNorRight = [brushContextDisRight.extent()[1],d3v0.time.minute.offset(brushContextDisRight.extent()[1],+facteurBrusNorRight) ];
		}
		break;
	case 1:
		facteurBrusNorLeft = (d3v0.time.hours(brushContextNorLeft.extent()[0],brushContextNorLeft.extent()[1])).length;
		facteurBrusNorRight = (d3v0.time.hours(brushContextNorRight.extent()[0],brushContextNorRight.extent()[1])).length;
		
		if(lockedLeft){
			extentNorLeft = [brushContextNorLeft.extent()[0], brushContextDisLeft.extent()[0] ];	
		}else{
			extentNorLeft = [d3v0.time.hour.offset(brushContextDisLeft.extent()[0],-facteurBrusNorLeft), brushContextDisLeft.extent()[0] ];
		}
		if(lockedRight){
			extentNorRight = [brushContextDisRight.extent()[1],brushContextNorRight.extent()[1] ];
		}else{
			extentNorRight = [brushContextDisRight.extent()[1],d3v0.time.hour.offset(brushContextDisRight.extent()[1],+facteurBrusNorRight) ];
		}
		
		break;
	case 2:
		facteurBrusNorLeft = (d3v0.time.days(brushContextNorLeft.extent()[0],brushContextNorLeft.extent()[1])).length;
		facteurBrusNorRight = (d3v0.time.days(brushContextNorRight.extent()[0],brushContextNorRight.extent()[1])).length;

		if(lockedLeft){
			extentNorLeft = [brushContextNorLeft.extent()[0], brushContextDisLeft.extent()[0] ];	
		}else{
			extentNorLeft = [d3v0.time.day.offset(brushContextDisLeft.extent()[0],-facteurBrusNorLeft), brushContextDisLeft.extent()[0] ];
		}
		if(lockedRight){
			extentNorRight = [brushContextDisRight.extent()[1],brushContextNorRight.extent()[1] ];
		}else{
			extentNorRight = [brushContextDisRight.extent()[1],d3v0.time.day.offset(brushContextDisRight.extent()[1],+facteurBrusNorRight) ];
		}
		break;
	case 3:
		facteurBrusNorLeft = (d3v0.time.weeks(brushContextNorLeft.extent()[0],brushContextNorLeft.extent()[1])).length;
		facteurBrusNorRight = (d3v0.time.weeks(brushContextNorRight.extent()[0],brushContextNorRight.extent()[1])).length;

		if(lockedLeft){
			extentNorLeft = [brushContextNorLeft.extent()[0], brushContextDisLeft.extent()[0] ];	
		}else{
			extentNorLeft = [d3v0.time.week.offset(brushContextDisLeft.extent()[0],-facteurBrusNorLeft), brushContextDisLeft.extent()[0] ];
		}
		if(lockedRight){
			extentNorRight = [brushContextDisRight.extent()[1],brushContextNorRight.extent()[1] ];
		}else{
			extentNorRight = [brushContextDisRight.extent()[1],d3v0.time.week.offset(brushContextDisRight.extent()[1],+facteurBrusNorRight) ];
		}		
		break;
	case 4:
		facteurBrusNorLeft = (d3v0.time.months(brushContextNorLeft.extent()[0],brushContextNorLeft.extent()[1])).length;
		facteurBrusNorRight = (d3v0.time.months(brushContextNorRight.extent()[0],brushContextNorRight.extent()[1])).length;

		if(lockedLeft){
			extentNorLeft = [brushContextNorLeft.extent()[0], brushContextDisLeft.extent()[0] ];	
		}else{
			extentNorLeft = [d3v0.time.month.offset(brushContextDisLeft.extent()[0],-facteurBrusNorLeft), brushContextDisLeft.extent()[0] ];
		}
		if(lockedRight){
			extentNorRight = [brushContextDisRight.extent()[1],brushContextNorRight.extent()[1] ];
		}else{
			extentNorRight = [brushContextDisRight.extent()[1],d3v0.time.month.offset(brushContextDisRight.extent()[1],+facteurBrusNorRight) ];
		}
		break;
	case 5:
		facteurBrusNorLeft = (d3v0.time.years(brushContextNorLeft.extent()[0],brushContextNorLeft.extent()[1])).length;
		facteurBrusNorRight = (d3v0.time.years(brushContextNorRight.extent()[0],brushContextNorRight.extent()[1])).length;

		if(lockedLeft){
			extentNorLeft = [brushContextNorLeft.extent()[0], brushContextDisLeft.extent()[0] ];	
		}else{
			extentNorLeft = [d3v0.time.year.offset(brushContextDisLeft.extent()[0],-facteurBrusNorLeft), brushContextDisLeft.extent()[0] ];
		}
		if(lockedRight){
			extentNorRight = [brushContextDisRight.extent()[1],brushContextNorRight.extent()[1] ];
		}else{
			extentNorRight = [brushContextDisRight.extent()[1],d3v0.time.year.offset(brushContextDisRight.extent()[1],+facteurBrusNorRight) ];
		}
		break;
	}
...
//			.attr("y2", heightVertical);
	...
	d3v0.select(this).call(brushContextNorLeft.extent(extent1));
...
function brushContextDisLeftMove() {
	...
	d3v0.select(this).call(brushContextDisLeft.extent(extent1));
...
function brushContextDisRightMove() {
	...
	d3v0.select(this).call(brushContextDisRight.extent(extent1));
...
function brushContextNorRightMove() {
	...
	d3v0.select(this).call(brushContextNorRight.extent(extent1));
			...
			for (var i = 0; i < dateRange.length; i++) {
				...
				for(var j=index_flat; j < leaf_node.values.length; j++){
					...
					if(raw_node.date >= date_range_begin && raw_node.date < date_range_end){
						num_value = num_value + raw_node.volume;
				...
				results.push({
					...
					"value":Math.cbrt(num_value),
					// "value":Math.sqrt(num_value),
					"text":text
				})
				//console.log(results);
			}
...
//	}
	...
	areaNormal = d3v0.svg.area()
			...
			.defined(function(d) {
						...
						switch (timePolarity) {
							case 0:
								var1 = d3v0.time.minute.offset(brushContextNorLeft.extent()[0], -nTimeGranularity);
								var2 = d3v0.time.minute.offset(brushContextNorLeft.extent()[1], nTimeGranularity);
								
								var3 = d3v0.time.minute.offset(brushContextNorRight.extent()[0], -nTimeGranularity);
								var4 = d3v0.time.minute.offset(brushContextNorRight.extent()[1], nTimeGranularity);
								//
								interpolationLeft1 = d3v0.time.minute.offset(brushContextNorLeft.extent()[0],-1*nTimeGranularity);
								interpolationLeft2 = d3v0.time.minute.offset(brushContextNorLeft.extent()[1],-2*nTimeGranularity);
								interpolationRight1 = d3v0.time.minute.offset(brushContextNorRight.extent()[0],+3*nTimeGranularity);
								interpolationRight2 = d3v0.time.minute.offset(brushContextNorRight.extent()[1],+1*nTimeGranularity);
								break;
							case 1:
								var1 = d3v0.time.hour.offset(brushContextNorLeft.extent()[0], -nTimeGranularity);
								var2 = d3v0.time.hour.offset(brushContextNorLeft.extent()[1], nTimeGranularity);
								var3 = d3v0.time.hour.offset(brushContextNorRight.extent()[0], -nTimeGranularity);
								var4 = d3v0.time.hour.offset(brushContextNorRight.extent()[1], nTimeGranularity);
								//
								interpolationLeft1 = d3v0.time.hour.offset(brushContextNorLeft.extent()[0],-1*nTimeGranularity);
								interpolationLeft2 = d3v0.time.hour.offset(brushContextNorLeft.extent()[1],-2*nTimeGranularity);
								interpolationRight1 = d3v0.time.hour.offset(brushContextNorRight.extent()[0],+3*nTimeGranularity);
								interpolationRight2 = d3v0.time.hour.offset(brushContextNorRight.extent()[1],+1*nTimeGranularity);
								break;
							case 2:
								var1 = d3v0.time.day.offset(brushContextNorLeft.extent()[0], -nTimeGranularity);
								var2 = d3v0.time.day.offset(brushContextNorLeft.extent()[1], nTimeGranularity);
								var3 = d3v0.time.day.offset(brushContextNorRight.extent()[0], -nTimeGranularity);
								var4 = d3v0.time.day.offset(brushContextNorRight.extent()[1], nTimeGranularity);			
								//
								interpolationLeft1 = d3v0.time.day.offset(brushContextNorLeft.extent()[0],-1*nTimeGranularity);
								interpolationLeft2 = d3v0.time.day.offset(brushContextNorLeft.extent()[1],-2*nTimeGranularity);
								interpolationRight1 = d3v0.time.day.offset(brushContextNorRight.extent()[0],+3*nTimeGranularity);
								interpolationRight2 = d3v0.time.day.offset(brushContextNorRight.extent()[1],+1*nTimeGranularity);
								break;
							case 3:
								var1 = d3v0.time.week.offset(brushContextNorLeft.extent()[0], -nTimeGranularity);
								var2 = d3v0.time.week.offset(brushContextNorLeft.extent()[1], nTimeGranularity);
								var3 = d3v0.time.week.offset(brushContextNorRight.extent()[0], -nTimeGranularity);
								var4 = d3v0.time.week.offset(brushContextNorRight.extent()[1], nTimeGranularity);	
								//
								interpolationLeft1 = d3v0.time.week.offset(brushContextNorLeft.extent()[0],-1*nTimeGranularity);
								interpolationLeft2 = d3v0.time.week.offset(brushContextNorLeft.extent()[1],-2*nTimeGranularity);
								interpolationRight1 = d3v0.time.week.offset(brushContextNorRight.extent()[0],+3*nTimeGranularity);
								interpolationRight2 = d3v0.time.week.offset(brushContextNorRight.extent()[1],+1*nTimeGranularity);
								break;
							case 4:
								var1 = d3v0.time.month.offset(brushContextNorLeft.extent()[0], -nTimeGranularity);
								var2 = d3v0.time.month.offset(brushContextNorLeft.extent()[1], nTimeGranularity);
								var3 = d3v0.time.month.offset(brushContextNorRight.extent()[0], -nTimeGranularity);
								var4 = d3v0.time.month.offset(brushContextNorRight.extent()[1], nTimeGranularity);	
								//
								interpolationLeft1 = d3v0.time.month.offset(brushContextNorLeft.extent()[0],-1*nTimeGranularity);
								interpolationLeft2 = d3v0.time.month.offset(brushContextNorLeft.extent()[1],-2*nTimeGranularity);
								interpolationRight1 = d3v0.time.month.offset(brushContextNorRight.extent()[0],+3*nTimeGranularity);
								interpolationRight2 = d3v0.time.month.offset(brushContextNorRight.extent()[1],+1*nTimeGranularity);
								break;
							case 5:
								var1 = d3v0.time.year.offset(brushContextNorLeft.extent()[0], -nTimeGranularity);
								var2 = d3v0.time.year.offset(brushContextNorLeft.extent()[1], nTimeGranularity);
								var3 = d3v0.time.year.offset(brushContextNorRight.extent()[0], -nTimeGranularity);
								var4 = d3v0.time.year.offset(brushContextNorRight.extent()[1], nTimeGranularity);	
								//
								
								interpolationLeft1 = d3v0.time.year.offset(brushContextNorLeft.extent()[0],-1*nTimeGranularity);
								interpolationLeft2 = d3v0.time.year.offset(brushContextNorLeft.extent()[1],-2*nTimeGranularity);
								interpolationRight1 = d3v0.time.year.offset(brushContextNorRight.extent()[0],+3*nTimeGranularity);
								interpolationRight2 = d3v0.time.year.offset(brushContextNorRight.extent()[1],+1*nTimeGranularity);
								break;
						}
//						return d.date;
	...
	/* LEFT DISTORTION */
	areaDistortionLeft = d3v0.svg.area()
				.interpolate(opts.interpolateType)
				.defined(function(d) {
						var var1, var2;
						...
						switch (timePolarity) {
							case 0:
								var1 = d3v0.time.minute.offset(brushContextDisLeft.extent()[0], -nTimeGranularity);
								var2 = d3v0.time.minute.offset(brushContextDisLeft.extent()[1], nTimeGranularity);
								//
								interpolationLeft1 = d3v0.time.minute.offset(brushContextDisLeft.extent()[0],-2*nTimeGranularity);
								interpolationLeft2 = d3v0.time.minute.offset(brushContextDisLeft.extent()[1],-2*nTimeGranularity);
								break;
							case 1:
								var1 = d3v0.time.hour.offset(brushContextDisLeft.extent()[0], -nTimeGranularity);
								var2 = d3v0.time.hour.offset(brushContextDisLeft.extent()[1], nTimeGranularity);
								//
								interpolationLeft1 = d3v0.time.hour.offset(brushContextDisLeft.extent()[0],-2*nTimeGranularity);
								interpolationLeft2 = d3v0.time.hour.offset(brushContextDisLeft.extent()[1],-2*nTimeGranularity);
								break;
							case 2:
								var1 = d3v0.time.day.offset(brushContextDisLeft.extent()[0], -nTimeGranularity);
								var2 = d3v0.time.day.offset(brushContextDisLeft.extent()[1], nTimeGranularity);
								//
								interpolationLeft1 = d3v0.time.day.offset(brushContextDisLeft.extent()[0],-2*nTimeGranularity);
								interpolationLeft2 = d3v0.time.day.offset(brushContextDisLeft.extent()[1],-2*nTimeGranularity);
								break;
							case 3:
								var1 = d3v0.time.week.offset(brushContextDisLeft.extent()[0], -nTimeGranularity);
								var2 = d3v0.time.week.offset(brushContextDisLeft.extent()[1], nTimeGranularity);
								//
								interpolationLeft1 = d3v0.time.week.offset(brushContextDisLeft.extent()[0],-2*nTimeGranularity);
								interpolationLeft2 = d3v0.time.week.offset(brushContextDisLeft.extent()[1],-2*nTimeGranularity);
								break;
							case 4:
								var1 = d3v0.time.month.offset(brushContextDisLeft.extent()[0], -nTimeGranularity);
								var2 = d3v0.time.month.offset(brushContextDisLeft.extent()[1], nTimeGranularity);
								//
								interpolationLeft1 = d3v0.time.month.offset(brushContextDisLeft.extent()[0],-2*nTimeGranularity);
								interpolationLeft2 = d3v0.time.month.offset(brushContextDisLeft.extent()[1],-2*nTimeGranularity);
								break;
							case 5:
								var1 = d3v0.time.year.offset(brushContextDisLeft.extent()[0], -nTimeGranularity);
								var2 = d3v0.time.year.offset(brushContextDisLeft.extent()[1], nTimeGranularity);
								//
								interpolationLeft1 = d3v0.time.year.offset(brushContextDisLeft.extent()[0],-2*nTimeGranularity);
								interpolationLeft2 = d3v0.time.year.offset(brushContextDisLeft.extent()[1],-2*nTimeGranularity);
								break;
						}
						if(interpolationLeft1 < dateMinRange && interpolationLeft2 < dateMinRange ){
							return false;
	...
	/* ZOOM */
	areaZoom = d3v0.svg.area() 
			.interpolate(opts.interpolateType)
			.defined(function(d) {
				var var1, var2;
				switch (timePolarity) {
					case 0:
						var1 = d3v0.time.minute.offset(brushContext.extent()[0],-nTimeGranularity);
						var2 = d3v0.time.minute.offset(brushContext.extent()[1],nTimeGranularity);
						break;
					case 1:
						var1 = d3v0.time.hour.offset(brushContext.extent()[0],-nTimeGranularity);
						var2 = d3v0.time.hour.offset(brushContext.extent()[1],nTimeGranularity);
						break;
					case 2:
						var1 = d3v0.time.day.offset(brushContext.extent()[0],-nTimeGranularity);
						var2 = d3v0.time.day.offset(brushContext.extent()[1],nTimeGranularity);
						break;
					case 3:
						var1 = d3v0.time.week.offset(brushContext.extent()[0],-nTimeGranularity);
						var2 = d3v0.time.week.offset(brushContext.extent()[1],nTimeGranularity);
						break;
					case 4:
						var1 = d3v0.time.month.offset(brushContext.extent()[0],-nTimeGranularity);
						var2 = d3v0.time.month.offset(brushContext.extent()[1],nTimeGranularity);
						break;
					case 5:
						var1 = d3v0.time.year.offset(brushContext.extent()[0],-nTimeGranularity);
						var2 = d3v0.time.year.offset(brushContext.extent()[1],nTimeGranularity);
						break;
				}
//				return d.date;
				return d.date >= (var1) && d.date <= (var2);
	...
	/* RIGHT DISTORTION */
	areaDistortionRight = d3v0.svg.area()
			.interpolate(opts.interpolateType)
			.defined(function(d) {
				var var1, var2;
				...
				switch (timePolarity) {
					case 0:
						var1 = d3v0.time.minute.offset(brushContextDisRight.extent()[0],-nTimeGranularity);
						var2 = d3v0.time.minute.offset(brushContextDisRight.extent()[1], nTimeGranularity);
						//
						interpolationRight1 = d3v0.time.minute.offset(brushContextDisRight.extent()[0],+4*nTimeGranularity);
						interpolationRight2 = d3v0.time.minute.offset(brushContextDisRight.extent()[1],+4*nTimeGranularity);
						break;
					case 1:	
						var1 = d3v0.time.hour.offset(brushContextDisRight.extent()[0],-nTimeGranularity);
						var2 = d3v0.time.hour.offset(brushContextDisRight.extent()[1], nTimeGranularity);
						//
						interpolationRight1 = d3v0.time.hour.offset(brushContextDisRight.extent()[0],+4*nTimeGranularity);
						interpolationRight2 = d3v0.time.hour.offset(brushContextDisRight.extent()[1],+4*nTimeGranularity);
						break;
					case 2:
						var1 = d3v0.time.day.offset(brushContextDisRight.extent()[0],-nTimeGranularity);
						var2 = d3v0.time.day.offset(brushContextDisRight.extent()[1], nTimeGranularity);
						//
						interpolationRight1 = d3v0.time.day.offset(brushContextDisRight.extent()[0],+4*nTimeGranularity);
						interpolationRight2 = d3v0.time.day.offset(brushContextDisRight.extent()[1],+4*nTimeGranularity);
						break;
					case 3:
						var1 = d3v0.time.week.offset(brushContextDisRight.extent()[0],-nTimeGranularity);
						var2 = d3v0.time.week.offset(brushContextDisRight.extent()[1], nTimeGranularity);
						//
						interpolationRight1 = d3v0.time.week.offset(brushContextDisRight.extent()[0],+4*nTimeGranularity);
						interpolationRight2 = d3v0.time.week.offset(brushContextDisRight.extent()[1],+4*nTimeGranularity);
						break;
					case 4:
						var1 = d3v0.time.month.offset(brushContextDisRight.extent()[0],-nTimeGranularity);
						var2 = d3v0.time.month.offset(brushContextDisRight.extent()[1], nTimeGranularity);
						//
						interpolationRight1 = d3v0.time.month.offset(brushContextDisRight.extent()[0],+4*nTimeGranularity);
						interpolationRight2 = d3v0.time.month.offset(brushContextDisRight.extent()[1],+4*nTimeGranularity);
						break;
					case 5:
						var1 = d3v0.time.year.offset(brushContextDisRight.extent()[0],-nTimeGranularity);
						var2 = d3v0.time.year.offset(brushContextDisRight.extent()[1], nTimeGranularity);
						//
						interpolationRight1 = d3v0.time.year.offset(brushContextDisRight.extent()[0],+4*nTimeGranularity);
						interpolationRight2 = d3v0.time.year.offset(brushContextDisRight.extent()[1],+4*nTimeGranularity);
						break;
				}
...
function calculeExtent(brushExtent, index) {
	...
	switch (timePolarity) {
		case 0:
			d0 = d3v0.time.minute.offset(brushExtent[0], index * nTimeGranularity );
			d1 = d3v0.time.minute.offset(brushExtent[0], index * nTimeGranularity + nTimeGranularity);
			break;
		case 1:
			d0 = d3v0.time.hour.offset(brushExtent[0], index * nTimeGranularity );
			d1 = d3v0.time.hour.offset(brushExtent[0], index * nTimeGranularity + nTimeGranularity);
			break;
		case 2:
			d0 = d3v0.time.day.offset(brushExtent[0], index * nTimeGranularity );
			d1 = d3v0.time.day.offset(brushExtent[0], index * nTimeGranularity + nTimeGranularity);			
			break;
		case 3:
			d0 = d3v0.time.week.offset(brushExtent[0], index * nTimeGranularity );
			d1 = d3v0.time.week.offset(brushExtent[0], index * nTimeGranularity + nTimeGranularity);			
			break;
		case 4:
			d0 = d3v0.time.month.offset(brushExtent[0], index * nTimeGranularity );
			d1 = d3v0.time.month.offset(brushExtent[0], index * nTimeGranularity + nTimeGranularity);				
			break;
		case 5:
			d0 = d3v0.time.year.offset(brushExtent[0], index * nTimeGranularity );
			d1 = d3v0.time.year.offset(brushExtent[0], index * nTimeGranularity + nTimeGranularity);				
			break;
	}
...
//	var barNorRight = dateRange[(indexDateAveRange + opts.factBrushContext/2 + opts.factBrushFisheye + opts.factBrushNormal)];
	...
	//DisLeft
	xScaleContextDisLeft = d3v0.time.scale()
	...
	brushContextDisLeft = d3v0.svg.brush()
	...
	//NorRight
	xScaleContextNorRight = d3v0.time.scale()
									.range([ xScaleContext(barDisRight), width ])
									.domain([ barDisRight, barNorRight ]);

	brushContextNorRight = d3v0.svg.brush()
	...
	//DisRight
	xScaleContextDisRight = d3v0.time.scale()
	...
	brushContextDisRight = d3v0.svg.brush()
