...
var opts_tree = {
	...
	strokeWidthNodeDeselected : 0.5
}


var margin = {top: 0, right: 120, bottom: 50, left: 60},
    width = 300 - margin.right - margin.left,
    height = 1050 - margin.top - margin.bottom; //screenHeight 1050

var tree = d3v0.layout.tree()
					.size([height, width]);

var diagonal = d3v0.svg.diagonal()
...
svg_tree_vis = d3v0.select("body").select("#tree-vis")
    .attr("width", (300))
...
function loadTreeVis(){
	...
	update(root);
	tooltip_tree = d3v0.select("#cm-menu-content").append("div")
												.attr("id", "tooltip-tree")
					
												
}

//d3v0.select(self.frameElement).style("height", "800px");

function zoom() {
    svgGroup.attr("transform", "translate(" + d3v0.event.translate + ")scale(" + d3v0.event.scale + ")");
}
...
function update(source) {
  ...
  d3v0.select("#tree-vis").on("mousemove",function(d,i){
	  
		var mouse_coordinates = d3v0.mouse(this);
...
//zoom and darg
d3.select("#tree-vis").call(d3.zoom()
    .scaleExtent([1 / 2, 8])
    .on("zoom", zoomed));
function zoomed() {
    d3.select("#tree-vis").select("g").attr("transform", d3.event.transform);
}
