/**
 * @fileOverview A d3v3 based distribution chart system. Supports: Box plots, Violin plots, Notched box plots, trend lines, beeswarm plot
 * @version 3.0
 */
...
var divplotchartpersondottooltip = d3.select("#div_persondot1").append("persondottooltip")
...
function makeDistroChart(settings) {
    ...
    !function prepareChart() {
        ...
        let divname = "#div_persondot1"
        let width= document.querySelector( divname ).clientWidth - 50
        let height = document.querySelector( divname).clientHeight - 50;
        chart.objs.mainDiv.append("div")
            .attr("class", "inner-wrapper")
            // .style("padding-bottom", (chart.divHeight / chart.divWidth) * 100 + "%")
            .style("padding-bottom",  document.querySelector( divname ).clientWidth )
            .style("padding-right",  document.querySelector( divname).clientHeight )
        ...
        for (var cName in chart.groupObjs) {
            ...
            chart.groupObjs[cName].g.on("mouseover", function () {
                chart.objs.tooltip
                    ...
                    .style("left", (d3v3.event.pageX-1400) + "px")
                    .style("top", (d3v3.event.pageY -150 ) + "px").style("opacity",0)
                    .style("width","100px").style("height","100px").style("opacity",0)
                    .style("background-color", "#eee")
                    .style("box-shadow", "0 0 5px #999999")
                    .style("border-width", "3px")
                    .style("border-radius", "3px")
            ...
            for (cName in chart.groupObjs) {
                ...
                if (cPlot.objs.points) {
                    if (dOpts.plotType == 'beeswarm') {
                        ...
                        for (var pt = 0; pt < cGroup.values.length; pt++) {
                            let index = cGroup.pidvalues.indexOf(cGroup.values[pt])
                            let pid = cGroup.pid[index]  //这是对value排过序的
                            ...
                            cPlot.objs.points.pts[pt]
                                ...
                                .on("click",function(){
                                    ...
                                    let selectpid_idname =  this.getAttribute("id")
                                    let dayvalue = selectpid_idname.substring(17,18);
                                    let pid = selectpid_idname.substring(18,selectpid_idname.length)
                                    if(dayvalue == "1"){
                                        severalpersonidtrackclick( pid,"red",1)
                                    }else if(dayvalue == "2"){
                                        severalpersonidtrackclick( pid,"blue",2)
                                    }else{
                                        severalpersonidtrackclick( pid,"yellow",3)
                                    }
                                    if(plotchartclickpidarray.indexOf(pid)){
                                        plotchartclickpidarray.push(pid)
                                    }
                                    barchartinputpidarray(plotchartclickpidarray)
                                })
                                .on("mouseover", function() {
                                    var coords = d3v3.mouse(this);
                                    let selectpid_idname =  this.getAttribute("id")
                                    let pid = selectpid_idname.substring(18,selectpid_idname.length)
                                    divplotchartpersondottooltip.transition().duration(200).style("visibility", "visible")
                                        .style("opacity", 1);
                                    divplotchartpersondottooltip.html( "person:  " + pid)
                                        .style("left", (coords[0]-65) + "px")
                                        .style("top", coords[1] + "px");
                                    d3.selectAll("#" + selectpid_idname)
                                        .attr("r",personiddotoselectedr).style("opacity", 1)
                                })
                                .on("mouseout", function() {
                                    let selectpid_idname =  this.getAttribute("id")
                                    divplotchartpersondottooltip.transition().duration(500).style("visibility", "hide")
                                        .style("opacity", 0);
                                    d3.selectAll("#"+ selectpid_idname ).attr("r",3)
                                        .style("opacity", 0.7)  })
                        }
                    }
                }
